...
void setDefault()
{
    ...
    {
	std::cerr << "tcsetattr() return with string : " << strerror(errno) << std::endl;
    }

}

void Serial::setControlModes(const unsigned &flags)
{
    tty.c_cflag &= flags;
}

void Serial::setInputModes(const unsigned &flags)
{
    tty.c_iflag &= flags;
}

void Serial::setOutputModes(const unsigned &flags)
{
    tty.c_oflag &= flags;
}

void Serial::setLocalModes(const unsigned &flags)
{
    tty.c_lflag &= flags;
}
